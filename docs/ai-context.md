...
| Business Config | `src/config/*.ts` (e.g., `chaty.ts`) | Do not hardcode UX copy in React components |
| API Orchestration | dedicated modules under `src/integrations/` | Do not place `fetch()` inside `.tsx` components |
...
If an agent needs to change the WhatsApp number, an assistant tagline, or a Notion DB ID for the Chaty hub, they must *only* edit `src/config/chaty.ts`.
...
Agents modifying any frontend integration wrapper must ensure that the payload schemas match what the corresponding n8n workflows expect.
